...
> **Config files required:** Promtail and Grafana read config from `./config/promtail.yml` and `./config/grafana/provisioning/`. These files are included in the repository. Use `git clone` to get them rather than copying the compose snippet manually.
...
- **GRAFANA_PASSWORD**: Admin password for Grafana at `https://yourdomain.com/grafana` (user: `admin`). Auto-generated by `setup.sh`. To change it: `docker compose exec grafana grafana-cli admin reset-admin-password <new-password>`.
...
All service logs are aggregated by Promtail → Loki and viewable in Grafana:

- **URL**: `https://yourdomain.com/grafana` (or `http://localhost:8080/grafana`)
- **Login**: user `admin`, password is `GRAFANA_PASSWORD` from `.env`
- Loki is pre-configured as the default datasource

Example LogQL queries:
```
# All logs from the BFF
{service="server"}

# Errors across all services
{service=~".+"} |= "error"

# Messaging service logs only
...
docker run --rm \
  ...
  -v tone-chat_loki-data:/data/loki \
  -v tone-chat_grafana-data:/data/grafana \
  -v $(pwd):/backup \
...
**Can't access Grafana (`/grafana` returns 404 or blank):**
- Check `docker compose logs grafana` for startup errors
- Verify `GRAFANA_PASSWORD` is set in `.env`
- Confirm `config/promtail.yml` and `config/grafana/provisioning/` exist (they ship with the repo)
