...
import { validateBody, createChannelSchema, updateChannelSchema } from '../shared/validate.js';

export const channelsRouter = Router({ mergeParams: true });

channelsRouter.post('/', mutationLimiters.serverWrite, validateBody(createChannelSchema), async (req: AuthRequest, res) => {
...
channelsRouter.patch('/:channelId', mutationLimiters.serverWrite, validateBody(updateChannelSchema), async (req: AuthRequest, res) => {
