...
describe("registerMessageHandlers", () => {
  ...
  describe("send_message", () => {
    it("calls createMessage and emits new_message on 201", async () => {
      ...
      mockCreateMessage.mock.mockImplementation(async () => ({
        status: 201,
        data: responseData,
      }));
    ...
    it("passes replyToId and mentions to createMessage", async () => {
      ...
      const responseData = {
        message: { _id: "m1", content: "hello", mentions: ["u2"] },
      };
      mockCreateMessage.mock.mockImplementation(async () => ({
        status: 201,
        data: responseData,
    ...
    it("calls emitMentionsFromResult after successful create", async () => {
      const responseData = {
        message: { _id: "m1", content: "hello", mentions: ["u2", "u3"] },
      };
      mockCreateMessage.mock.mockImplementation(async () => ({
        status: 201,
        data: responseData,
  ...
  describe("typing", () => {
    it("broadcasts typing event to the room", () => {
      ...
      assert.equal(
        socket.to.mock.calls[0]!.arguments[0],
        "server:s1:channel:c1",
      );
      assert.equal(toEmit.mock.calls[0]!.arguments[0], "typing");
      assert.deepEqual(toEmit.mock.calls[0]!.arguments[1], {
        userId: "user-1",
        channelId: "c1",
      });
    });
  ...
  describe("toggle_reaction", () => {
    it("calls toggleReaction and broadcasts reaction_updated on 200", async () => {
      ...
      mockToggleReaction.mock.mockImplementation(async () => ({
        status: 200,
        data: responseData,
      }));
    ...
    it("does not broadcast on non-200 status", async () => {
      ...
      await handlers["toggle_reaction"]!({
        serverId: "s1",
        channelId: "c1",
        messageId: "m1",
        emoji: "👍",
