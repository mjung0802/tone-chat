...
- **messages.socket.ts** — `registerMessageHandlers()` — Socket.IO handlers: `send_message` (validates content 1–4000 chars, max 6 attachments, max 20 mentions; rate-limited 30/min per user), `typing`, `toggle_reaction` (rate-limited 60/min per user)
