...
- **dms.routes.ts** — BFF HTTP proxy routes with rate limiting + bidirectional block enforcement; validates mutation request bodies via `validateBody(createMessageSchema)`, `validateBody(editMessageSchema)`, `validateBody(reactionSchema)`; delegates broadcast/notification to `dms.broadcast.ts`
