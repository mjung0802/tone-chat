...
function listenOnEphemeral(app: { listen: (port: number) => HttpServer }): {
  server: HttpServer;
  url: string;
} {
...
before(async () => {
  ...
  const channelModelPath =
    "../../messagingService/src/channels/channel.model.js";
  const messageModelPath =
    "../../messagingService/src/messages/message.model.js";
  const memberModelPath =
    "../../messagingService/src/members/serverMember.model.js";
  const inviteModelPath = "../../messagingService/src/invites/invite.model.js";

  // 1. Start usersService (needs DATABASE_URL for PG)
  process.env["DATABASE_URL"] =
    "postgres://tone:tone_dev@localhost:5442/tone_users_test";
  ...
  process.env["DATABASE_URL"] =
    "postgres://tone:tone_dev@localhost:5443/tone_attachments_test";
  ...
  (config as { messagingServiceUrl: string }).messagingServiceUrl =
    messagingUrl;
  (config as { attachmentsServiceUrl: string }).attachmentsServiceUrl =
    attachmentsUrl;
...
describe("BFF Servers", () => {
  it("creates server with auto #general channel", async () => {
    ...
    const { serverId, channelId } = await createTestServer(
      accessToken,
      "My Server",
    );
    assert.ok(serverId);
...
describe("BFF Channels", () => {
  it("creates channel and lists sorted by position", async () => {
    ...
    const listRes = await fetch(
      `${bffUrl}/api/v1/servers/${serverId}/channels`,
      {
        headers: authHeaders(accessToken),
      },
    );
    const body = (await listRes.json()) as {
      channels: Array<{ name: string; position: number }>;
    };
...
describe("BFF Messages", () => {
  ...
  it("author-only edit", async () => {
    ...
    const { message } = (await createRes.json()) as {
      message: { _id: string };
    };
    ...
    assert.equal(res.status, 200);
    const body = (await res.json()) as {
      message: { content: string; editedAt: string };
    };
...
describe("BFF Members", () => {
  ...
  it("joins public server and returns 409 on duplicate", async () => {
    ...
    const joinRes = await fetch(
      `${bffUrl}/api/v1/servers/${server._id}/members`,
      {
        method: "POST",
        headers: authHeaders(bob.accessToken),
      },
  ...
  it("self-leave", async () => {
    ...
    const leaveRes = await fetch(
      `${bffUrl}/api/v1/servers/${server._id}/members/${bob.user.id}`,
      {
        method: "DELETE",
        headers: authHeaders(bob.accessToken),
      },
...
describe("BFF Invites", () => {
  it("creates invite and joins via code", async () => {
    ...
    const joinRes = await fetch(
      `${bffUrl}/api/v1/invites/${invite.code}/join`,
      {
        method: "POST",
        headers: authHeaders(bob.accessToken),
      },
...
describe("BFF Attachments", () => {
  it("uploads file and retrieves metadata", async () => {
    ...
    const res = await fetch(
      `${bffUrl}/api/v1/attachments/upload?filename=test.txt`,
      {
        method: "POST",
        headers: {
          authorization: `Bearer ${accessToken}`,
    ...
    assert.equal(res.status, 201);
    const body = (await res.json()) as {
      attachment: { id: string; filename: string; status: string };
    };
    ...
    const getRes = await fetch(
      `${bffUrl}/api/v1/attachments/${body.attachment.id}`,
      {
        headers: authHeaders(accessToken),
      },
    );
    ...
    assert.ok(
      getBody.attachment.url,
      "presigned URL should be present on retrieval",
    );
  });
  ...
  it("creates message with attachment IDs", async () => {
    ...
    const uploadRes = await fetch(
      `${bffUrl}/api/v1/attachments/upload?filename=doc.txt`,
      {
        method: "POST",
        headers: {
          authorization: `Bearer ${accessToken}`,
    ...
    const msgBody = (await msgRes.json()) as {
      message: { content: string; attachmentIds: string[] };
    };
    ...
    const listBody = (await listRes.json()) as {
      messages: Array<{ attachmentIds: string[] }>;
    };
    assert.ok(
      listBody.messages.some((m) => m.attachmentIds.includes(attachment.id)),
    );
  });
});
