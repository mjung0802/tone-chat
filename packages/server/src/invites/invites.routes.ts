...
serverInvitesRouter.post('/', async (req: AuthRequest, res) => {
  const result = await client.createInvite(req.userId!, req.params['serverId'] as string, req.body as Record<string, unknown>);
  res.status(result.status).json(result.data);
});

serverInvitesRouter.get('/', async (req: AuthRequest, res) => {
  const result = await client.listInvites(req.userId!, req.params['serverId'] as string);
  res.status(result.status).json(result.data);
});
...
joinRouter.post('/:code/join', async (req: AuthRequest, res) => {
  const result = await client.joinViaInvite(req.userId!, req.params['code'] as string);
  res.status(result.status).json(result.data);
});
