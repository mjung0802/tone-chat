...
bansRouter.get('/', async (req: AuthRequest, res) => {
  const result = await client.listBans(req.token!, req.params['serverId'] as string);
  res.status(result.status).json(result.data);
});

bansRouter.delete('/:userId', async (req: AuthRequest, res) => {
  const result = await client.unbanUser(req.token!, req.params['serverId'] as string, req.params['userId'] as string);
