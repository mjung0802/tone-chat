...
describe("POST /attachments/upload", () => {
  ...
  it("uploads an image file", async () => {
    ...
    assert.equal(res.status, 201);
    const body = (await res.json()) as {
      attachment: { mime_type: string; filename: string };
    };
...
describe("GET /attachments/:id", () => {
  it("returns attachment metadata with presigned URL", async () => {
    ...
    formData.append(
      "file",
      new Blob([Buffer.from("data")], { type: "text/plain" }),
      "doc.txt",
    );
    ...
    const body = (await res.json()) as {
      attachment: { id: string; filename: string; status: string; url: string };
    };
  ...
  it("returns 404 for non-existent ID", async () => {
    const res = await fetch(
      `${baseUrl}/attachments/00000000-0000-0000-0000-000000000000`,
      {
        headers: HEADERS,
      },
    );
