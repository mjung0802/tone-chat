...
- **useInvites.ts** — `useInvites()`, `useDefaultInvite()`, `useCreateInvite()`, `useRevokeInvite()`, `useJoinViaCode()` (invalidates `['servers']` + `['invite-status']` on success), `useInviteStatus(code)`
