...
function mockUseAttachment(
  overrides: Partial<
    ReturnType<typeof useAttachmentsModule.useAttachment>
  > = {},
) {
  const defaults = {
    data: undefined,
  ...
  jest
    .mocked(useAttachmentsModule.useAttachment)
    .mockReturnValue(
      defaults as ReturnType<typeof useAttachmentsModule.useAttachment>,
    );
}
...
describe("AttachmentBubble", () => {
  ...
  it("opens URL when file card is pressed", () => {
    ...
    expect(Linking.openURL).toHaveBeenCalledWith(
      "http://localhost:9000/uploads/report.pdf",
    );
  });
