...
describe('AttachmentViewer', () => {
  ...
  it('does not render modal when attachment has no URL', () => {
    ...
    const { queryByLabelText } = renderWithProviders(
      <AttachmentViewer visible={true} attachment={attachment} onClose={jest.fn()} />,
    );
  ...
  it('close button calls onClose', () => {
    ...
    const { getByLabelText } = renderWithProviders(
      <AttachmentViewer visible={true} attachment={attachment} onClose={onClose} />,
    );
  ...
  it('renders image with correct source URI', () => {
    ...
    expect(image.props.source).toEqual({ uri: 'http://localhost:9000/uploads/banner.jpg' });
  });
});
