...
jest.mock("./AttachmentBubble", () => {
  ...
  return {
    AttachmentBubble: ({ attachmentId }: { attachmentId: string }) => {
      return (
        <Text testID={`attachment-${attachmentId}`}>AttachmentBubble</Text>
      );
    },
  };
});
...
jest.mock("../common/UserAvatar", () => {
  ...
  return {
    UserAvatar: ({
      ...
      return (
        <Text testID="user-avatar">{`UserAvatar:${name}:${avatarAttachmentId ?? "none"}:${size ?? 32}`}</Text>
      );
    },
  };
});
    ...
    ReactionChips: (props: {
      ...
      return (
        <View testID="reaction-chips">
          <Text testID="reaction-count">
            {(props.reactions as unknown[]).length}
          </Text>
          <Pressable
            testID="mock-toggle"
