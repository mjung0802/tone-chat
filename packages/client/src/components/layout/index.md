...
- **ServerRail.tsx** — 60px vertical nav rail: home button with total DM unread badge, top-5 DM unread avatars (DmRailAvatar), scrollable server icon list (active server highlighted via `contained-tonal` mode, derived from `usePathname`), profile/logout buttons at bottom
