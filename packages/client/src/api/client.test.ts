...
describe('API client', () => {
  ...
  it('throws ApiClientError with code/message/status on 4xx', async () => {
    ...
    const err = await get('/bad').catch((e: unknown) => e) as InstanceType<typeof ApiClientError>;
  ...
  it('throws with code=UNKNOWN when body is not JSON', async () => {
    ...
    const err = await get('/err').catch((e: unknown) => e) as { code: string; status: number };
  ...
  it('on 401 when refresh fails (non-OK): calls clearAuth', async () => {
    ...
    mockFetch
      .mockResolvedValueOnce(mockResponse(401, { error: { code: 'UNAUTHORIZED', message: 'bad', status: 401 } }))
      .mockResolvedValueOnce(mockResponse(403, { error: { code: 'FORBIDDEN', message: 'revoked', status: 403 } }));
  ...
  it('retry request (isRetry=true) does NOT re-trigger refresh', async () => {
    ...
    mockFetch
      .mockResolvedValueOnce(mockResponse(401, { error: { code: 'UNAUTHORIZED', message: 'no', status: 401 } }))
      .mockResolvedValueOnce(
        mockResponse(200, { accessToken: 'new-at', refreshToken: 'new-rt' }),
      )
      .mockResolvedValueOnce(
        mockResponse(401, { error: { code: 'UNAUTHORIZED', message: 'still bad', status: 401 } }),
      );
