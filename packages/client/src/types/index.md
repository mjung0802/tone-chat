...
- **models.ts** — `User`, `Server`, `Channel`, `Message`, `CustomToneDefinition`, `ServerMember`, `ServerBan`, `Invite`, `Attachment`, `DirectConversation`, `DirectMessage`, `AuditLogEntry` interfaces; `AuditAction` type
- **api.types.ts** — request types (RegisterRequest, LoginRequest, SendDmRequest, UpdateDmRequest, ToggleDmReactionRequest, etc.), response wrappers (AuthResponse, RefreshResponse, DirectConversationResponse, DirectConversationsResponse, DirectMessageResponse, DirectMessagesResponse, BlockedIdsResponse, AuditLogResponse, etc.), query/filter types (MessagesQuery, AuditLogQuery, etc.)
