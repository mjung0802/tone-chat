...
- **notificationStore.ts** — `useNotificationStore` — mention notification state (currentNotification, currentChannelId), quiet/system preferences; DM unread count (dmUnreadCount, incrementDmUnread(), clearDmUnread()); `hydrateNotificationPreference()`
