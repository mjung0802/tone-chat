...
async function gotoVerifyEmailScreen(
  page: Parameters<typeof mockUnverifiedLoginRoute>[0],
) {
...
test("login with unverified account shows verify-email screen", async ({
  page,
}) => {
...
test("resend button starts cooldown and becomes disabled after click", async ({
  page,
}) => {
  await gotoVerifyEmailScreen(page);
  ...
  await expect(
    page.getByLabel(/Resend code available in \d+ seconds/),
  ).toBeDisabled();
});
