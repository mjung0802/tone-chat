...
const { register, login, refresh, verifyEmail, resendVerification } =
...
describe("register", () => {
  ...
  it("returns 400 when username missing", async () => {
    const res = makeRes();
    await register(
      makeReq({ body: { email: "a@b.com", password: "12345678" } }),
      res,
    );
  ...
  it("returns 400 when email missing", async () => {
    const res = makeRes();
    await register(
      makeReq({ body: { username: "alice", password: "12345678" } }),
      res,
    );
    assert.equal(res.statusCode, 400);
  });
  ...
  it("returns 400 when password missing", async () => {
    const res = makeRes();
    await register(
      makeReq({ body: { username: "alice", email: "a@b.com" } }),
      res,
    );
    assert.equal(res.statusCode, 400);
  });
  ...
  it("returns 201 on success", async () => {
    ...
    await register(
      makeReq({
        body: { username: "alice", email: "a@b.com", password: "12345678" },
      }),
      res,
    );
...
describe("verifyEmail", () => {
  ...
  it("calls verifyOtp with userId from x-user-id header and code from body", async () => {
    ...
    const res = makeRes();
    await verifyEmail(
      makeReq({ headers: { "x-user-id": "u1" }, body: { code: "123456" } }),
      res,
    );
  ...
  it("propagates AppError from verifyOtp", async () => {
    ...
    await assert.rejects(
      () =>
        verifyEmail(
          makeReq({ headers: { "x-user-id": "u1" }, body: { code: "000000" } }),
          res,
        ),
...
describe("resendVerification", () => {
  ...
  it('returns 200 { message: "Verification email sent" } on success', async () => {
    ...
    assert.equal(res.statusCode, 200);
    assert.equal(
      (res._json as { message: string }).message,
  ...
  it("propagates AppError from getUserById", async () => {
    ...
    await assert.rejects(
      () =>
        resendVerification(
          makeReq({ headers: { "x-user-id": "nonexistent" } }),
          res,
        ),
