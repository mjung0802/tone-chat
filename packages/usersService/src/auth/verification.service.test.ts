...
const { sendVerificationOtp, verifyOtp } = await import('./verification.service.js');
...
describe('sendVerificationOtp', () => {
  ...
  it('deletes existing tokens before inserting new one', async () => {
    ...
    assert.ok((sqlStrings[0] ?? '').trim().startsWith('DELETE'), 'first SQL call should be DELETE');
  });
  ...
  it('generates a 6-digit zero-padded code when randomInt returns 5', async () => {
    ...
    assert.equal(mockSendVerificationEmail.mock.calls[0]!.arguments[1], '000005');
  });
  ...
  it('calls sendVerificationEmail with correct email and code', async () => {
    ...
    assert.equal(mockSendVerificationEmail.mock.calls[0]!.arguments[0], 'user@test.com');
    assert.equal(mockSendVerificationEmail.mock.calls[0]!.arguments[1], '000005');
  });
  ...
  it('propagates SMTP failure (does not suppress)', async () => {
    ...
    await assert.rejects(() => sendVerificationOtp('user-1', 'user@test.com'), /SMTP failed/);
  });
});
...
describe('verifyOtp', () => {
  ...
  it('calls UPDATE users SET email_verified on success', async () => {
    ...
    mockSql.mock.mockImplementation(() => {
      callCount++;
      if (callCount === 1) return [{ expires_at: new Date(Date.now() + 60_000) }];
      return [];
    });
    ...
    assert.ok(sqlText.includes('email_verified'), 'should update email_verified');
  });
