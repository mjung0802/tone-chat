...
- **internal.routes.ts** — `internalRouter` — `POST /bidirectional-check` — checks if either of two users blocks the other; requires only `internalAuth` (no user token needed)
