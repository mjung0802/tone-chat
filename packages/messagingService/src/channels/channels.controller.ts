...
export async function createChannel(req: Request, res: Response): Promise<void> {
  ...
  const { name, type, topic } = req.body as { name: string; type?: string; topic?: string };
  ...
  const maxPos = await Channel.findOne({ serverId }).sort({ position: -1 }).select('position');
  const position = maxPos ? maxPos.position + 1 : 0;

  const channel = await Channel.create({ serverId, name, type: type ?? 'text', topic, position });
...
export async function updateChannel(req: Request, res: Response): Promise<void> {
  ...
  const { name, topic, position } = req.body as { name?: string; topic?: string; position?: number };
...
export async function deleteChannel(req: Request, res: Response): Promise<void> {
  const result = await Channel.findOneAndDelete({ _id: req.params['channelId'], serverId: req.params['serverId'] });
  if (!result) {
    throw new AppError('CHANNEL_NOT_FOUND', 'Channel not found', 404);
  }
  res.status(204).end();
