...
const { createInvite, listInvites, revokeInvite, joinViaInvite } =
  await import("./invites.controller.js");
...
describe("createInvite", () => {
  ...
  it("sets expiresAt from expiresIn and returns 201", async () => {
    ...
    const createArg = mockInviteCreate.mock.calls[0]!.arguments[0] as {
      expiresAt: Date;
    };
...
describe("listInvites", () => {
  it("queries with revoked: false and returns 200", async () => {
    ...
    assert.equal(
      (mockInviteFind.mock.calls[0]!.arguments[0] as { revoked: boolean })
        .revoked,
      false,
    );
  });
...
describe("joinViaInvite", () => {
  ...
  it("throws INVITE_NOT_FOUND when invite is null", async () => {
    ...
    await assert.rejects(
      () =>
        joinViaInvite(
          makeReq({ headers: { "x-user-id": "u1" }, params: { code: "bad" } }),
          makeRes(),
        ),
  ...
  it("throws INVITE_REVOKED when revoked", async () => {
    ...
    await assert.rejects(
      () =>
        joinViaInvite(
          makeReq({ headers: { "x-user-id": "u1" }, params: { code: "abc" } }),
          makeRes(),
        ),
