...
const { createMessage, listMessages, updateMessage } =
  await import("./messages.controller.js");
...
describe("listMessages", () => {
  ...
  it("defaults limit to 50 and returns reversed messages", async () => {
    ...
    const res = makeRes();
    await listMessages(
      makeReq({ params: { channelId: "c1" }, query: {} }),
      res,
    );
    assert.equal(res.statusCode, 200);
    // messages.reverse() is called
    assert.deepEqual((res._json as { messages: unknown[] }).messages, [
      { _id: "m1" },
      { _id: "m2" },
    ]);
  });
  ...
  it("caps limit at 100", async () => {
    ...
    const res = makeRes();
    await listMessages(
      makeReq({ params: { channelId: "c1" }, query: { limit: "999" } }),
      res,
    );
  ...
  it("applies cursor pagination with before", async () => {
    ...
    const res = makeRes();
    await listMessages(
      makeReq({ params: { channelId: "c1" }, query: { before: "cursor-id" } }),
      res,
    );
    assert.deepEqual((capturedFilter as Record<string, unknown>)._id, {
      $lt: "cursor-id",
    });
  });
});
...
describe("createMessage — mentions", () => {
  beforeEach(() => {
    ...
    mockMessageCreate.mock.mockImplementation(async (...args: unknown[]) => ({
      _id: "m1",
  ...
  it("stores mentions array when provided", async () => {
    ...
    assert.equal(res.statusCode, 201);
    const createArg = mockMessageCreate.mock.calls[0]!.arguments[0] as Record<
      string,
      unknown
    >;
...
describe("createMessage — replyTo", () => {
  ...
  it("uses nickname for authorName when available", async () => {
    ...
    >;
    assert.equal(
      (createArg.replyTo as { authorName: string }).authorName,
      "CoolNick",
    );
  });
  ...
  it("truncates replyTo content to 100 chars", async () => {
    ...
    >;
    assert.equal(
      (createArg.replyTo as { content: string }).content.length,
      100,
    );
  });
});
