...
type MessageWithContentResponse = {
  messages: Array<{ content: string; reactions: unknown[] }>;
};
...
describe("Reactions", () => {
  it("adds a reaction and returns it in GET", async () => {
    ...
    const putRes = await fetch(
      `${baseUrl}/servers/${serverId}/channels/${channelId}/messages/${msg._id}/reactions`,
      {
        method: "PUT",
    ...
    assert.equal(
      (putBody.message.reactions[0] as Record<string, unknown>).emoji,
      "\u{1F44D}",
    );
    assert.deepEqual(
      (putBody.message.reactions[0] as Record<string, unknown>).userIds,
      ["user-1"],
    );
  ...
  it("multiple users react with same emoji", async () => {
    ...
    assert.equal(
      (
        (body.message.reactions[0] as Record<string, unknown>)
          .userIds as unknown[]
      ).length,
      2,
    );
  ...
  it("react to nonexistent message returns 404", async () => {
    ...
    const res = await fetch(
      `${baseUrl}/servers/${serverId}/channels/${channelId}/messages/000000000000000000000000/reactions`,
      {
        method: "PUT",
        headers: headersFor("user-1"),
        body: JSON.stringify({ emoji: "\u{1F44D}" }),
  ...
  it("reactions persist across message edits", async () => {
    ...
    await fetch(
      `${baseUrl}/servers/${serverId}/channels/${channelId}/messages/${msg._id}`,
      {
        method: "PATCH",
        headers: headersFor("user-1"),
    ...
    assert.equal(
      (body.messages[0]!.reactions[0] as Record<string, unknown>).emoji,
      "\u{1F44D}",
    );
  });
});
