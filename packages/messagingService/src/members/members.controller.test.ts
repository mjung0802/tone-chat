...
const { joinServer, listMembers, getMember, updateMember, removeMember } =
  await import("./members.controller.js");
...
describe("getMember", () => {
  ...
  it("throws MEMBER_NOT_FOUND when null", async () => {
    ...
    await assert.rejects(
      () =>
        getMember(
          makeReq({ params: { serverId: "s1", userId: "u1" } }),
          makeRes(),
        ),
...
describe("removeMember", () => {
  ...
  it("allows self-leave without admin check", async () => {
    mockMemberFindOneAndDelete.mock.mockImplementation(async () => ({
      userId: "u1",
    }));
    const res = makeRes();
    await removeMember(
