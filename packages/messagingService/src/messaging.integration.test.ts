...
describe("Server lifecycle", () => {
  it("POST creates server with auto #general channel and admin member", async () => {
    ...
    const body = (await res.json()) as {
      server: { _id: string; name: string; ownerId: string };
    };
...
describe("Channel lifecycle", () => {
  ...
  it("PATCH and DELETE are admin-only", async () => {
    ...
    const res = await fetch(
      `${baseUrl}/servers/${serverId}/channels/${channelId}`,
      {
        method: "PATCH",
        headers: headersFor("user-2"),
  ...
  it("DELETE: admin can delete, non-admin cannot", async () => {
    ...
    const { channel } = (await createRes.json()) as {
      channel: { _id: string };
    };
    ...
    const res = await fetch(
      `${baseUrl}/servers/${serverId}/channels/${channel._id}`,
      {
        method: "DELETE",
        headers: headersFor("user-2"),
      },
...
describe("Message lifecycle", () => {
  ...
  it("GET returns messages with cursor pagination", async () => {
    ...
    // Fetch with limit
    const res = await fetch(
      `${baseUrl}/servers/${serverId}/channels/${channelId}/messages?limit=3`,
      {
        headers: headersFor("user-1"),
      },
    );
    ...
    const res2 = await fetch(
      `${baseUrl}/servers/${serverId}/channels/${channelId}/messages?limit=3&before=${cursor}`,
      {
        headers: headersFor("user-1"),
      },
    );
  ...
  it("PATCH is author-only", async () => {
    ...
    // Author can edit
    const res = await fetch(
      `${baseUrl}/servers/${serverId}/channels/${channelId}/messages/${message._id}`,
      {
        method: "PATCH",
        headers: headersFor("user-1"),
...
describe("Invite lifecycle", () => {
  ...
  it("GET lists active invites; non-admin cannot list", async () => {
    ...
    const { invite: invite1 } = (await res1.json()) as {
      invite: { code: string };
    };
    ...
    const { invite: invite2 } = (await res2.json()) as {
      invite: { code: string };
    };
